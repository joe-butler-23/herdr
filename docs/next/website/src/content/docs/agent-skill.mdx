...
- inspect workspaces, tabs, and non-worker support panes
- split support panes and run commands without stealing focus
- read server, test, and log output
- wait for process readiness or completion

Worker spawning, mail, and cleanup are deliberately not taught in the skill. Official Claude Code, Codex, and OpenCode integrations inject that separate doctrine only into sessions running inside Herdr.
...
The full operator guide lives in the skill file itself. It covers volatile pane IDs, `pane split`, `pane run`, `pane read`, `wait output`, support-process recipes, and workspace and tab commands.
