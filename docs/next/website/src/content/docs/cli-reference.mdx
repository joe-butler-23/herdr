...
`agent start` places the new pane deterministically when `--tab` is given, and otherwise falls back to UI focus. Run from inside a herdr pane, an omitted `--workspace`/`--tab` instead defaults to the calling pane's own workspace/tab (from its `HERDR_WORKSPACE_ID`/`HERDR_TAB_ID` env vars), so a worker spawned by an orchestrator lands with its caller rather than wherever a human happens to be looking; explicit `--workspace`/`--tab` flags always override the env default. Outside a pane (no env vars present), placement keeps following UI focus as before.
...
herdr integration check <target>
```

`integration check` is an exit-code gate: it returns zero only when the selected integration is current. OpenCode additionally requires exact installed plugin bytes and the absence of its legacy global delegation-doctrine block.
