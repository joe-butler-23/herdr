...
### Added
- Added `herdr mail send`, `herdr mail wait`, `herdr mail read`, and `herdr mail list` for inter-agent message coordination via an in-memory message bus, avoiding token-expensive polling; messages are classified as `done`, `blocked`, `question`, or `info` with per-recipient inbox delivery and envelope/body separation. `mail wait`/`mail list`/`mail read` take `--inbox` to select whose inbox to act on (default: the caller's own pane, from `HERDR_PANE_ID`); `mail wait`/`mail list` additionally take `--from <sender>` to filter to a single sender's mail (a pane id, agent name, or terminal id), so waiting on one worker's reply is not satisfied by another in-flight worker's mail. When a caller sends mail without an explicit `from_agent` (as the plain CLI does), the server now populates it from the resolved sender pane's detected/reported agent label — the same label `agent.list` shows — instead of leaving it blank; an explicit `from_agent` (as hook callers pass) still wins.
...
- Added automatic mail delivery from Claude Code, Codex, and OpenCode integrations when spawned as delegated workers (with a parent pane); workers send `done` mail when their turn completes (or `done (no message)` when a turn produced no assistant text) and `blocked` mail on permission waits.
- Added a shared delegation doctrine, taught in SKILL.md and installed by `herdr integration install claude|codex|opencode` as a herdr-managed block in each agent's global instructions file (`~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, `~/.config/opencode/AGENTS.md`, created if absent, removed on uninstall), so codex and opencode orchestrators get the same argv-launch and mail-wait guidance Claude Code gets instead of improvising launch-then-type spawns or pane-polling loops. The doctrine also now tells workers to never `pane run`/`pane send-text` into a parent or any other pane to coordinate — that injects fake user input rather than delivering mail — and to reply by mail to the sender's pane id instead; tells orchestrators that a parented worker auto-mails `done` at the end of every turn, including a turn where it only asked a question, so completion must be judged by the mail's content rather than its kind alone; and clarifies that `--from` filters match the sender's terminal identity, so a pane/terminal id is more reliable than an agent-name filter once more than one worker shares an agent label. The waiting guidance is now inverted from an earlier draft that led with a foreground `mail wait`: ending the turn right after dispatch is THE method, correct on every harness, since herdr's mail-arrival nudge wakes the orchestrator with nothing to wait for or poll; backgrounding `mail wait` is now stated as the one narrow exception, available only when the harness can run a shell command in the background and notify on completion (claude code; not codex or opencode), with flat prohibitions on foreground waits, on treating `--timeout` as a polling interval, and on re-issuing waits in a loop. Also added an orchestrator duty to close a worker's pane (`herdr pane close <pane_id>`) once its final done-mail is read and integrated, unless the same worker is about to be reused. Bumped the claude, codex, and opencode integration versions (11, 11, 12) accordingly.
- Added guidance for launching delegated workers with the task as argv in one atomic `herdr agent start <agent> ... -- <cmd> "<task>"` command instead of launching bare and typing the prompt in afterward (racy: missed Enter keys and boot races), plus the same inverted waiting guidance — end the turn right after dispatch as the default for every harness, and background the wait only as the claude-code-only exception. `herdr mail wait --help` now notes that mail arrival nudges the recipient's pane directly, so agents should normally end their turn instead of waiting, since a foreground wait blocks the caller's turn for up to `--timeout`.
- Added a mail-arrival nudge that wakes an idle recipient agent instead of leaving delivered mail unnoticed: when mail arrives for a pane whose agent is idle or done, herdr types a `[herdr mail] <kind> from <sender> "<subject>" — check: herdr mail read <id>` line plus Enter into that pane; while the agent is working or blocked the nudge queues and is delivered on the transition to idle, multiple unread messages collapse to a single `[herdr mail] N unread — check: herdr mail list --unread-only` line, only panes with a detected/reported agent are nudged (never plain shells), and a queued nudge is cancelled if the agent reads its mail first. For a single unread `question`, `info`, or `blocked` message the line also appends a reply hint pointing at the sender's pane id from the envelope, e.g. `; reply: herdr mail send <from_pane_id> --kind info --subject "..."` (falling back to the sender's terminal id if no pane id was recorded), so a nudged worker replies by mail instead of typing into the orchestrator's pane; `done` mail keeps the plain form since no reply is expected. Gated globally by `[mail] nudge = true` in config.toml (default on) and per message with `herdr mail send --no-nudge`.
- Added `--consume` to `herdr mail wait` to atomically mark the matched message read before returning its envelope; without it a wait never marks anything read, so a re-armed wait re-returns the same message until `mail read` (or `--consume`) consumes it.
...
### Fixed
- Codex's Stop-hook fallback for a turn that produced no assistant text now carries `(no message)` in the mail body as well as the subject, so a reader that only looks at the body no longer sees an empty string.
