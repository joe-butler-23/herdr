...
- Added a shared delegation doctrine, taught in SKILL.md and injected only into Herdr-managed Claude Code, Codex, and OpenCode sessions. Claude and Codex receive it through their session hooks; OpenCode receives it through the plugin's `experimental.chat.system.transform` hook when `HERDR_ENV=1`. Installing any of these integrations removes its older Herdr-managed global-instructions block, and OpenCode installation preserves unrelated `AGENTS.md` content. The doctrine standardizes atomic argv worker launches, mail-only coordination, completion judgement, non-polling waits, and worker-pane cleanup. Added `herdr integration check <target>` as an exit-code gate; for OpenCode, current means exact plugin bytes and no legacy global doctrine block. Bumped the Claude, Codex, and OpenCode integration versions (11, 11, 12) accordingly.
