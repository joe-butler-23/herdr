...
- Added a shared delegation doctrine as the sole authority for worker spawning, mail, and cleanup, injected only into Herdr-managed Claude Code, Codex, and OpenCode sessions. The public `SKILL.md` now remains a progressive operator guide for workspaces, tabs, and non-worker support panes instead of duplicating worker coordination. Claude and Codex receive the doctrine through their session hooks on Unix and Windows; OpenCode receives it through the plugin's `experimental.chat.system.transform` hook when `HERDR_ENV=1`. Installing any of these integrations removes its older Herdr-managed global-instructions block, and OpenCode installation preserves unrelated `AGENTS.md` content. The doctrine standardizes atomic argv worker launches, mail-only coordination, completion judgement, non-polling waits, and worker-pane cleanup. Added `herdr integration check <target>` as an exit-code gate; current returns 0, known missing or outdated state returns 1, and operational check failures return 2. For OpenCode, current means exact plugin bytes and no legacy global doctrine block. Bumped the Claude, Codex, and OpenCode integration versions to 12 accordingly.
