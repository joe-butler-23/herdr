...
- Added a shared session doctrine as the sole authority for operating inside Herdr, replacing the redundant repository and global Herdr skills. Claude Code and Codex receive it through their session hooks on Unix and Windows; OpenCode receives it through the plugin's `experimental.chat.system.transform` hook when `HERDR_ENV=1`. The doctrine covers orchestrator and worker identity, atomic worker launches, mail-only worker coordination, completion judgement, worker cleanup, live-ID handling, and safe control of non-agent support panes. Installing any of these integrations removes its older Herdr-managed global-instructions block, while OpenCode installation preserves unrelated `AGENTS.md` content. Added `herdr integration check <target>` as an exit-code gate; current returns 0, known missing or outdated state returns 1, and operational check failures return 2. For Claude Code, Codex, and OpenCode, current requires exact rendered content, required registrations, and no legacy global doctrine block. Bumped the three integration versions to 12 accordingly.
- Added guidance for launching delegated workers with the task as argv in one atomic `herdr agent start <agent> ... -- <cmd> "<task>"` command instead of launching bare and typing the prompt afterwards, plus a uniform non-polling rule: dispatch independent workers, end the turn, and let Herdr mail wake the orchestrator. `herdr mail wait --help` notes that mail arrival nudges the recipient's pane directly, so a foreground wait normally blocks useful progress for up to `--timeout`.
